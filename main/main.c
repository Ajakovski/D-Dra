                                ...
                                int32_t event_id, void *event_data)
{
    if (base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        // Static IP: no DHCP, so set the bit here instead of IP_EVENT
        ESP_LOGI(WIFI_TAG, "Associated. Static IP: 172.20.10.5");
        xEventGroupSetBits(wifi_event_group, WIFI_CONNECTED_BIT);
    } else if (base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        // Keep this for DHCP mode compatibility
        ip_event_got_ip_t *e = (ip_event_got_ip_t *)event_data;
        ESP_LOGI(WIFI_TAG, "IP (DHCP): " IPSTR, IP2STR(&e->ip_info.ip));
        xEventGroupSetBits(wifi_event_group, WIFI_CONNECTED_BIT);
    }
}

static void wifi_set_static_ip(void)
{
    esp_netif_t *netif = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
    if (!netif) {
        ESP_LOGE(TAG, "Could not get netif handle");
        return;
    }

    // Stop DHCP client before setting static config
    ...
    IP4_ADDR(&ip_info.ip,      172, 20, 10,  5);   // Drone static IP
    IP4_ADDR(&ip_info.gw,      172, 20, 10,  1);   // iPhone hotspot gateway
    IP4_ADDR(&ip_info.netmask, 255, 255, 255, 240); // /28 subnet
    ...
    // Set DNS (use gateway as DNS, or 8.8.8.8 if you need internet resolution)
    esp_netif_dns_info_t dns;
    IP4_ADDR(&dns.ip.u_addr.ip4, 172, 20, 10, 1);
    dns.ip.type = ESP_IPADDR_TYPE_V4;
    esp_netif_set_dns_info(netif, ESP_NETIF_DNS_MAIN, &dns);

    ESP_LOGI(TAG, "Static IP set: 172.20.10.5 / 255.255.255.240 GW 172.20.10.1");
}
...
static void wifi_init(void)
{
    ...
    // Set static IP before connecting — skips DHCP entirely
    wifi_set_static_ip();
...
{
    ESP_LOGI(TAG, "══════════════════════════════════════════════");
    ESP_LOGI(TAG, "  ESP32-S3 Drone Controller  v1.6.0");
    ...
    // ── FIX: Init ESC hardware FIRST ──────────────────────────────────────
    // This drives GPIO1/5/10/11 to 1000µs immediately — before any delay.
    // ESCs that power up now will see a valid disarmed signal from t=0.
    // The FC task enters DISARMED state and calls esc_all_to_min() each loop
    // — safe to run without IMU data since it never reads IMU while disarmed.
    if (flight_control_init() != ESP_OK) {
        ESP_LOGE(TAG, "FLIGHT CONTROL INIT FAILED — halting");
        while (1) vTaskDelay(portMAX_DELAY);
    }

    // IMU calibration runs AFTER ESC pins are actively driven — no floating
    ...
    if (!(bits & WIFI_CONNECTED_BIT)) {
        ESP_LOGE(TAG, "DHCP timeout — no IP after 15s. Check hotspot.");
    }
